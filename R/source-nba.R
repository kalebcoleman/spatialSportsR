#' NBA source helpers (internal)
#'
#' @description
#' Internal ESPN NBA source implementation returned by `get_source("nba")`.
#'
#' @keywords internal
...
source_nba <- function() {
  list(
    ...
    normalize_xy = function(x, y) {
      # Default NBA court normalization (feet: 94x50).
      data.frame(
        x_unit = x / 94,
        y_unit = y / 50
      )
  ...
  for (team_block in teams) {
    ...
    for (cat in categories) {
      ...
      for (ath in athletes) {
        ...
        stat_values <- as.list(stats)
        if (length(keys) > 0) {
          names(stat_values) <- keys[seq_len(min(length(keys), length(stat_values)))]
        }
...
.espn_nba_split_compound_stats <- function(stat_values, drop_compound = FALSE) {
  if (is.null(stat_values) || length(stat_values) == 0) return(stat_values)
  compound_keys <- names(stat_values)[grepl("-", names(stat_values))]
  if (length(compound_keys) == 0) return(stat_values)

  for (key in compound_keys) {
    value <- stat_values[[key]]
    if (is.null(value)) next
    value_chr <- as.character(value)
    if (!grepl("^[0-9]+-[0-9]+$", value_chr)) next
    parts <- strsplit(value_chr, "-", fixed = TRUE)[[1]]
    if (length(parts) != 2) next
    made <- suppressWarnings(as.integer(parts[[1]]))
    att <- suppressWarnings(as.integer(parts[[2]]))
    new_keys <- strsplit(key, "-", fixed = TRUE)[[1]]
    if (length(new_keys) == 2) {
      stat_values[[new_keys[[1]]]] <- made
      stat_values[[new_keys[[2]]]] <- att
    }
    if (isTRUE(drop_compound)) {
      stat_values[[key]] <- NULL
    }
  }
...
.espn_nba_parse_plays <- function(x) {
  ...
  rows <- lapply(plays, function(play) {
    ...
    x_unit <- NA_real_
    y_unit <- NA_real_
    if (is.numeric(x_raw) && is.numeric(y_raw) && !is.na(x_raw) && !is.na(y_raw)) {
      if (x_raw >= 0 && x_raw <= 94 && y_raw >= 0 && y_raw <= 50) {
        x_unit <- x_raw / 94
        y_unit <- y_raw / 50
      }
    }
    ...
    participant_ids <- participant_ids[!is.na(participant_ids) & participant_ids != ""]
    ...
    data.frame(
      ...
      x_raw = suppressWarnings(as.numeric(x_raw)),
      y_raw = suppressWarnings(as.numeric(y_raw)),
      participant_ids = if (length(participant_ids) == 0) NA_character_ else paste(participant_ids, collapse = ","),
      stringsAsFactors = FALSE,
      check.names = FALSE
    )
  })
...
.espn_nba_manifest_from_file <- function(path, x, has_events = NA, has_team_box = NA, has_player_box = NA, has_teams = NA) {
  ...
  .espn_nba_as_df(list(
    ...
    season = season,
    game_id = game_id,
    downloaded_at = if (!is.na(fi$mtime)) format(fi$mtime, "%Y-%m-%d %H:%M:%S") else NA_character_,
    source_url = NA_character_,
    status_code = NA_integer_,
    bytes = if (!is.na(fi$size)) as.integer(fi$size) else NA_integer_,
...
.espn_nba_empty_events <- function() {
  .espn_nba_as_df(data.frame(
    ...
    scoring_play = logical(0),
    shooting_play = logical(0),
    points_attempted = integer(0),
